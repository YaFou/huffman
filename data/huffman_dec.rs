...
pub fn encode_file(in_file: String, out_file: String) -> Result<(), Error> {
    ...
    writeln!(file, "{} {}", contents.chars().count(), map.len())?;

    for (c, v) in map.iter() {
        writeln!(file, "{} {}", c, bit_vec_to_string(v.to_vec()))?;
    }
...
pub fn decode_file(in_file: String, out_file: String) -> Result<(), Error> {
    ...
    while chars_processed < distinct_chars_count {
        if lines[line_index] == "" {
            line_index += 1;
            map.insert(
                string_to_bit_vec(lines[line_index].trim().to_string()),
                '\n',
            );
        } else {
            let c = lines[line_index].chars().nth(0).unwrap();
            let (_, code) = lines[line_index].split_at(2);
            map.insert(string_to_bit_vec(code.trim().to_string()), c);
        }
        line_index += 1;
        chars_processed += 1;
    }
